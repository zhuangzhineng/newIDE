...
window.common = {
	...
	getAppetizePublicKey:function (device) {
		if (window.localStorage.appid) {
			var platform = this.getPlatformWithDevice(device);
			var key = this.getAppid() + '_' + platform;
			return window.localStorage[key];
		} else {
			return null;
		}
	},
	setAppetizePublicKey:function (pk, platform) {
		var key = this.getAppid() + '_' + platform;
		window.localStorage[key] = pk;
	},
	getPlatformWithDevice:function(device) {
		if (device == 'nexus5' || device == 'nexus7' || device == 'nexus9') {
			return 'android';
		} else {
			return 'ios'
		}
